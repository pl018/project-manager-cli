"""Configuration module for the project manager."""
...
class ConfigManager:
    ...
    def __init__(self):
        ...
        self.config_dir = base_dir / 'project-manager-cli'
    ...
    def get_default_db_path(self) -> Path:
        ...
        if os.name == 'nt':  # Windows
            app_data_dir = Path(os.environ.get('APPDATA', Path.home() / '.config'))
        else:  # Unix-like
            app_data_dir = Path.home() / '.config'

        app_data_dir = app_data_dir / 'project-manager-cli'
    ...
    def get_default_projects_file(self) -> Path:
        ...
        return base_path / 'Cursor' / 'User' / 'globalStorage' / 'alefragnani.project-manager' / 'projects.json'
...
# Global config instance for dynamic configuration
